...
for (i in c(1:9)) {
    ...
    comp_sub_long$center_cell_type[is.na(comp_sub_long$center_cell_type)]='not labeled'
    nearclass=comp_sub_long$cell_members[comp_sub_long$center_cell_type=="Tumor Classical (1,2,3,5,7,8,11,14)"]
    ...
    classfibs=as.integer(fibids %in% nearclass)
    basfibs=as.integer(fibids %in% nearbasal)
    fibrobin=classfibs*2+basfibs ## 0=neither, 1=basal, 2=classical, 3=both
    ...
    bas_v_class=FindMarkers(fibro_seur, ident.1=1, ident.2=2,
                           min.pct=0.1,
                           logfc.threshold=0.25,
                           test.use = "wilcox")
    bas_v_class$name=rownames(bas_v_class)
    bas_v_class$comp='basal v classical'
    ...
    class_v_neither=FindMarkers(fibro_seur, ident.1=2, ident.2=0,
                               min.pct=0.1,
                               logfc.threshold=0.25,
                               test.use = "wilcox")
    class_v_neither$name=rownames(class_v_neither)
    class_v_neither$comp='classical v neither'
    
    all1=rbind(bas_v_class, bas_v_neither, class_v_neither)
    ...
    neartumor=comp_sub_long$cell_members[!comp_sub_long$center_cell_type=='not labeled']
    farfibs=!fibids %in% neartumor
    
    fibrobin[farfibs]=-1 ## 0=neither, 1=basal, 2=classical, 3=both
    fibro_seur@meta.data$fibrobin=fibrobin
    fibro_seur=Seurat::SetIdent(fibro_seur, value="fibrobin")
    ...
    class_v_far=FindMarkers(fibro_seur, ident.1=2, ident.2=-1,
                           min.pct=0.1,
                           logfc.threshold=0.25,
                           test.use = "wilcox")
    class_v_far$name=rownames(class_v_far)
    class_v_far$comp='classical v no nearby tumor'
    
    all2=rbind(bas_v_class, bas_v_far, class_v_far)
...
pdf(paste0(home_path, 'man_plots/diff_exp_classical1.pdf'), w=9, h=3.5)
for (i in c(1:9)) {
...
pdf(paste0(home_path, 'man_plots/diff_exp_classical2.pdf'), w=9, h=3.5)
for (i in c(1:9)) {
...
write.csv(fibrosde1[abs(fibrosde1$avg_log2FC)>=.75,], paste0(home_path, 'man_plots/diff_exp_classical1.csv'))
write.csv(fibrosde2[abs(fibrosde2$avg_log2FC)>=.75,], paste0(home_path, 'man_plots/diff_exp_classical2.csv'))

write.csv(fibrosde1, paste0(home_path, 'man_plots/diff_exp_classical1_full.csv'))
write.csv(fibrosde2, paste0(home_path, 'man_plots/diff_exp_classical2_full.csv'))

write.csv(fibrosde1[fibrosde1$nhops==3,], paste0(home_path, 'man_plots/diff_exp_classical1_3hop.csv'))
write.csv(fibrosde2[fibrosde1$nhops==3,], paste0(home_path, 'man_plots/diff_exp_classical2_3hop.csv'))

fibrosde1$sig=abs(fibrosde1$avg_log2FC)>=.75
fibrosde1_sig=fibrosde1[fibrosde1$sig,]
